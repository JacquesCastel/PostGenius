...
function buildPrompt(user, theme, campaignContext = "", veille = null, remarks = []) {
  const ctx = userContextBlock(user) + remarksPromptBlock(remarks);
...
${writingRulesPrompt(user.defaultMaxChars || 1300)}
...
export async function generateText(user, theme, campaignContext, veille) {
  ...
  const res = await fetch("https://api.anthropic.com/v1/messages", {
    ...
    body: JSON.stringify({
      ...
      messages: [{ role: "user", content: buildPrompt(user, theme, campaignContext, veille, remarks) }],
    }),
  });
