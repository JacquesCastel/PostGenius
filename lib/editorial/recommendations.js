...
import { getRemarks, remarksRecommendationsBlock } from "../remarks";
...
function buildPrompt({ user, pillars, digest, count, remarks = [] }) {
...
Favorise les sujets liés aux premiers de cette liste quand c'est pertinent — sans t'y forcer à chaque fois.`
  ...
  return `Voici le profil du client :${profile}${keywordsBlock}${remarksRecommendationsBlock(remarks)}
...
export async function generateRecommendations(userId, { count = 3, force = false } = {}) {
  ...
  // Statut donné aux anciennes recos "proposée" quand une nouvelle fournée les remplace.
  // "ignorée" (bouton "Voir d'autres propositions") compte comme un rejet dans
  // l'apprentissage ; "remplacée" (régénération automatique) reste neutre.
  let replacedStatus = force ? "ignorée" : null;
  if (!force) {
    ...
    if (fresh) {
      // Une remarque ajoutée après la dernière fournée l'invalide : on régénère au prochain
      // chargement pour que les propositions en tiennent compte sans action de l'utilisateur.
      const lastRemark = await prisma.postRemark.findFirst({
        where: { userId },
        orderBy: { createdAt: "desc" },
        select: { createdAt: true },
      });
      if (!lastRemark || lastRemark.createdAt <= fresh.createdAt) return getActiveRecommendations(userId);
      replacedStatus = "remplacée";
    }
  }
  ...
  const { parsed, usage } = await callClaude(buildPrompt({ user, pillars, digest, count, remarks }));
  logUsage(userId, {
  ...
  if (replacedStatus) {
    await prisma.editorialRecommendation.updateMany({
      where: { userId, status: "proposée" },
      data: { status: replacedStatus, respondedAt: new Date() },
    });
  }
