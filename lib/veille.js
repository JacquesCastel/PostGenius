...
async function fetchUrl(url) {
  ...
  try {
    const res = await fetch(url, {
      signal: ctrl.signal,
      redirect: "follow",
      headers: { "User-Agent": "LinkeePost/1.0 (veille RSS)", Accept: "application/rss+xml, application/atom+xml, text/html, */*" },
    });
...
function decode(s) {
  return (s ?? "")
    ...
    .replace(/&#0?39;|&apos;/g, "'")
    .replace(/&nbsp;/g, " ")
    // Décodage des entités AVANT le retrait des balises : certains flux
    // (ex. Social Media Today) échappent doublement leur HTML
    // (&lt;p&gt; au lieu de <p>), qui ne redevient une vraie balise
    // qu'après ce décodage.
    .replace(/<[^>]+>/g, " ")
    .replace(/\s+/g, " ")
    .trim();
}
