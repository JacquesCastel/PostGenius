...
export async function GET(req) {
  ...
  if (error || !code) {
    return NextResponse.redirect(`${appUrl}?linkedin=org_refused`);
  }
  ...
  try {
    ...
    const res = NextResponse.redirect(`${appUrl}?linkedin=org_connected`);
    res.cookies.delete("li_org_oauth_state");
    return res;
  } catch (e) {
    console.error("Erreur callback org:", e);
    return NextResponse.redirect(`${appUrl}?linkedin=org_error`);
  }
}
